	...
	(string-append
		...
		;...
	      ;  "printf(\"START tc\");\nfflush(stdout);"
		    ...
		    ;     "printf(\"\\n$| 2::FP=: %ld | $\", FP);\n"
		    ;     "printf(\"\\n$| 2::STACK(FP)=FP=: %ld | $\", STACK(FP));\n"
		    ;    "printf(\"\\n$| 2::R6=: %ld | $\", R6);\n"
		    ;     "printf(\"\\n$| 2::STACK(R6)=: %ld | $\", STACK(R6));\n"
		    ;    "printf(\"\\nsource-R7: %ld ::: R6-dest: %ld \", R7, R6);\n"
			
			; cover the old frame with the new one
			; for each parameter up to "n" + n itself, enc, ret
			...
			; update fp - old fp (of the new frame)
			;"\tMOV(SP,"  ")\n"
