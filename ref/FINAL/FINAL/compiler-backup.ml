...
module Semantics : SEMANTICS = struct
	...
	let box_check param body =
		...
		let rec get_check param body =
			...
			| Set' (Var' (VarBound' (param, _, _)), expr) | Set' (Var' (VarParam' (param, _)), expr) -> get_check param expr
	...
	let compile_scheme_file scm_source_file asm_target_file =
		...
		let make_symbol_table c_table =
			let rec collect_symbols table lst = match table with
				| (Symbol s, base_address, value)::cdr -> collect_symbols cdr (base_address::lst)
				| _ -> lst in
			let symbol_list = collect_symbols c_table [] in
			symbol_list in
		...
		let cisc_functions =
			...
			let is_positive =
				...
				let core_asm =
					...
					"L_" ^ label ^ "_end:\n" ^
					"POP(R1);\n" in
				gen_wrapper "positive?" label 1 1 core_asm in
			...
			let divide =
				...
				let core_asm =
					...
					"MUL(INDD(R0, 1), INDD(R1, 1));\n" ^
					"MUL(INDD(R0, 2), INDD(R1, 2));\n" ^
					"INCR(R2);\n" ^
			...
			let string_to_symbol =
				...
				let core_asm =
					...
					"L_" ^ label ^ "_loop:\n" ^
					"CMP(INDD(R2, 0), IMM(0));\n" ^
					"JUMP_EQ(L_" ^ label ^ "_make_new_symbol);\n" ^
					"PUSH(R1);\n" ^
					"PUSH(INDD(R2, 1));\n" ^
					...
					"JUMP_EQ(L_" ^ label ^ "_loop);\n" ^

					"L_" ^ label ^ "_make_new_symbol:\n" ^
					"PUSH(FPARG(2));\n" ^
		...
		let scheme_functions =
			...
			let lt = "(define < (letrec ((lt (lambda (lst)
					                           (if (null? (cdr lst))
					                               #t
					                               (and (positive? (- (car (cdr lst)) (car lst)))
					                                    (lt (cdr lst)))))))
					              (lambda (fst . lst)
					                (if (null? lst)
					                    #t
